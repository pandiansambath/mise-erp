...
async def create_item(
    ...
    _: User = Depends(require("inventory:write")),
) -> ItemOut:
    item = await service.create_item(db, **payload.model_dump(exclude_none=True))
...
async def list_items(
    ...
    items = await service.list_items(db, category=category)
...
async def low_stock(
    ...
    items = await service.low_stock_items(db)
    return [
        LowStockAlert(
            item_id=i.id,
            name=i.name,
...
async def record_movement(
    ...
    if payload.movement_type == MovementType.PURCHASE_IN.value and payload.unit_cost is None:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "unit_cost is required for PURCHASE_IN"
        )
    try:
