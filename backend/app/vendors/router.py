...
async def export_vendors_csv(
    ...
    # The supplier list with emails, VAT numbers and payment terms is the asset
    # a departing manager walks out with. It left no trace at all until now.
    await audit.record(
        db, hotel_id=user.hotel_id, user=user, action="vendors.export",
        summary=f"Exported {len(rows)} suppliers",
    )
    return _list_file(
...
class _VendorsRowsIn(BaseModel):
...
async def preview_vendors_rows(
    payload: _VendorsRowsIn,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(require("vendors:write")),
) -> dict:
    ...
    allowed = {f.key for f in lists.VENDORS.fields}
    ...
    return list_io.classify(cleaned, lists.VENDORS, existing).as_dict()


@router.post("/import/read-ai")
async def read_vendors_with_ai(
    ...
    for f in files:
        ...
        if docbytes.is_unreadable(name):
            ...
            continue
        try:
            got = await ingest.read_as(data, f.content_type or "", name, "vendors")
    ...
    if not rows:
        return {
            ...
            "failed": failed,
            "why": (
                "I read those, but couldn't find any vendors in them. If they "
    ...
    existing = await service.list_vendors(db, user.hotel_id, active_only=False)
    plan = list_io.classify(rows[:list_commit.MAX_COMMIT_ROWS], lists.VENDORS, existing)
    await audit.record(
        db, hotel_id=user.hotel_id, user=user, action="vendors.read_ai",
        summary=f"AI read {len(read)} document(s) as vendors ({len(rows)} rows, nothing saved yet)",
    )
    return {
        "plan": plan.as_dict(),
        "read": read,
