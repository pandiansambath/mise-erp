...
#: Bind parameters `cloud_cost_daily` spends per row in a multi-VALUES insert:
#: day, service, usage_type, record_type, amount_usd, quantity, source, as_of,
#: is_estimate — plus `id`, which counts because it is a PYTHON-side
#: `default=uuid.uuid4` and therefore travels as a parameter rather than being
#: generated by the server. Ten, and the tenth is the easy one to miss.
PARAMS_PER_ROW = 10

#: PostgreSQL's wire protocol carries the parameter count as a signed 16-bit
#: integer, so 32,767 is the ceiling and asyncpg refuses anything above it.
PG_MAX_BIND_PARAMS = 32_767

#: 2,000 rows = 20,000 parameters, a little under two-thirds of the limit. The
#: headroom is deliberate: a column added to this table later raises
#: PARAMS_PER_ROW, and a chunk size computed to sit at 99% would turn that
#: routine migration into a production write failure.
UPSERT_CHUNK = 2_000
...
async def upsert_cloud_cost(db: Any, rows: list[dict]) -> int:
    ...
    ⚠️ CHUNKED, AND THE CHUNK SIZE IS ARITHMETIC RATHER THAN TASTE.
    ------------------------------------------------------------------------
    asyncpg refuses a statement with more than **32,767 bind parameters** — a
    hard limit in the PostgreSQL wire protocol, where the parameter count is a
    signed 16-bit integer. One multi-VALUES insert spends `PARAMS_PER_ROW` of
    them per row, so a single statement tops out just over three thousand rows.

    That is not a theoretical ceiling. The real first-run backfill of two
    calendar months measured 2,836 rows — 87% of the limit — and it only fit
    because the account was idle for the first three weeks of July. At the
    current ~51 rows a day, a fully active 78-day window is around 4,000 rows,
    and the whole write would fail with a driver error on THE ONE RUN THAT
    MATTERS MOST: the first, the one that fills in the history, and the one that
    only happens once. The page would go back to showing a dash, which is the
    exact bug this collector was written to fix.

    Chunking here rather than shortening the backfill, because the backfill is
    the feature.
    """
    from sqlalchemy.dialects.postgresql import insert
    ...
    with internal():
        for i in range(0, len(rows), UPSERT_CHUNK):
            stmt = insert(CloudCostDaily).values(rows[i : i + UPSERT_CHUNK])
            stmt = stmt.on_conflict_do_update(
            ...
            await db.execute(stmt)
        # ONE COMMIT for every chunk. A day's bill is one fact, and half of it
        # written is worse than none of it: the page would show a plausible
        # total that is quietly short. Every chunk succeeds or the lot rolls
        # back.
        await db.commit()
    return len(rows)
