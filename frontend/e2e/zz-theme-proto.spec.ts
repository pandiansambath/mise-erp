...
const SERVICE_BRAND_L: Record<string, string> = {
...
const PROTOS: Record<string, Proto> = {
  ...
  "service-dark": {
    label: "Service (Dark)",
    light: false,
    brand: { ...SERVICE_BRAND_L, "300": "#7fd8cf", "400": "#4cc0b4", "500": "#2aa196", "600": "#1b8178", "700": "#13645d" },
...
/** The semantic triad - deliberately NOT the brand accent. */
...
test("prototypes", async ({ browser }) => {
  ...
  const res = await boot.request.post(LIVE + "/api/auth/login", {
  ...
  if (page.url().includes("/login")) {
    // the session did not take - put it back and try once more
    await page.evaluate((t: string) => localStorage.setItem("mise_token", t), tok);
    await page.goto(LOCAL + "/dashboard", { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(8000);
  ...
  for (const k of Object.keys(PROTOS)) {
    ...
    await page.screenshot({ path: path.join(OUT, "proto-dash--" + k + ".png") });
    console.log("proto dash", k);
  }
  ...
  for (const k of Object.keys(PROTOS)) {
    await apply(page, k);
    await page.waitForTimeout(900);
    await page.screenshot({ path: path.join(OUT, "proto-inv--" + k + ".png") });
    console.log("proto inv", k);
  }
  await ctx.close();
});
